...
// this package is a bit of a hack. the rtp.Packetizer interface doesn't support
// specifying explicit timestamps on the packets it produces to guarantee
// monotonicity, but if we source data from another source we want to pass the packets
// through while still packetizing down to the webrtc mtu. therefore, we shoehorn
// in a dts packetizer by hijacking the samples argument on the packetizer.
